...
/**
 * Fixture di test per migrazione Akka 2.5.x -> 2.6.x.
 *
 * Contiene intenzionalmente API/idiomi "legacy" che in 2.6 risultano deprecati:
 * - ActorMaterializer / ActorMaterializerSettings
 * - TimerScheduler.startPeriodicTimer
 * - Scheduler.schedule
 *
 * Scopo: dare a Uplift una classe ricca di pattern da migrare.
 */
public class TestAkka1 {
    ...
    // --------------------------------------------------------------------------------------------
    // 1) STREAMS - ActorMaterializer (2.5 style) + ActorMaterializerSettings.withSupervisionStrategy
    // --------------------------------------------------------------------------------------------

    @Test
    public void shouldRunStreamUsingActorMaterializer_andMaterializerSettings() throws Exception {

        // Decider: su ArithmeticException fa Resume, altrimenti Stop
        scala.Function1<Throwable, Supervision.Directive> decider;
        decider =exc -> {
        ...
        // Stream con un errore intenzionale (divisione per zero) per esercitare supervision
        CompletionStage<List<Integer>> result =
                Source.range(1, 5)
                        .map(i -> 10 / (i - 3)) // i=3 -> ArithmeticException
    ...
    // --------------------------------------------------------------------------------------------
    // 2) SCHEDULER - system.scheduler().schedule(...) (deprecato da 2.6)
    // --------------------------------------------------------------------------------------------
    ...
    public static class LegacySchedulerActor extends AbstractActor {
        ...
        public void preStart() {
            ...
            cancellable = context().system().scheduler().scheduleAtFixedRate(
                    initialDelay,
                    interval,
                    self(),
                    new Tick(),
    ...
    // --------------------------------------------------------------------------------------------
    // 3) TIMERS - AbstractActorWithTimers + startPeriodicTimer (deprecato da 2.6)
    // --------------------------------------------------------------------------------------------
    ...
    public void shouldUseTimers_startPeriodicTimer_legacyApi() {
        new TestKit(system) {{
            ...
            // ricevo almeno un tick generato dal timer
        ...
        public Receive createReceive() {
            return receiveBuilder()
                    .match(Start.class, s -> {
                        ...
                        getTimers().startTimerAtFixedRate(TIMER_KEY, new TimerTick(), interval);
                    })
    ...
    // --------------------------------------------------------------------------------------------
    // 4) ASK PATTERN - PatternsCS.ask + Timeout + CompletionStage
    // --------------------------------------------------------------------------------------------
